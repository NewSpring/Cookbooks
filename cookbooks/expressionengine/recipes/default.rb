...
ruby_block "Amend Environment Config File" do
  block do
    ...
    file.search_file_replace_line("\['username']","$env_db['hostname'] = '#{node[:ee][:username]}';")
    file.search_file_replace_line("\['password']","$env_db['hostname'] = '#{node[:ee][:password]}';")
    file.search_file_replace_line("\['database']","$env_db['hostname'] = '#{node[:ee][:datebase]}';")
    file.write_file
  end
end
