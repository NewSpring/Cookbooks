name             'newrelic-ng'
maintainer       'Chris Aumann'
maintainer_email 'me@chr4.org'
license          'GNU Public License 3.0'
description      'Installs/Configures newrelic monitoring'
long_description IO.read(File.join(File.dirname(__FILE__), 'README.md'))
version          '0.3.3'
depends          'apt'
depends          'build-essential'
depends          'python'

%w{ ubuntu debian redhat centos amazon suse scientific }.each do |os|
  supports os
end

recipe "newrelic-ng::default", "installs newrelic"
recipe "newrelic-ng::plugin-agent-default", "installs newrelic plugin agent"

attribute "newrelic-ng/license_key",
  :display_name =>  "NewRelic License Key",
  :description => "License key to connect server/app to NewRelic Monitoring",
  :required => "required",
  :recipes => [
    "newrelic-ng::default",
    "newrelic-ng::plugin-agent-default"
  ]
